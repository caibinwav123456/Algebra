...
void BigInt::Clone(const MemBase& m, bool recur)
{
	...
	pos=b->pos;
	slitr(inner,vector<uchar>,b->inner.toType());
}
Wrapper<MemBase> BigInt::CloneR(const MemBase& m, bool recur) const
{
	BigInt* newint=new BigInt;
	newint->Clone(m,recur);
	return Wrapper<MemBase>(newint);
}
...
int Divide(const BigInt& a, const BigInt& b, BigInt& q, BigInt& m)
{
	...
	slit(q.inner,vector<uchar>);
	slit(m.inner,vector<uchar>);
	q.pos=(a.pos==b.pos);
	m.pos=b.pos;
